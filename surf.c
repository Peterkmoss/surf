...
static gboolean loadfailedtls(WebKitWebView *v, gchar *uri,
                              GTlsCertificate *cert, GTlsCertificateFlags err,
                              Client *c);
...
static ParamName loadtransient[] = {
    Certificate,         CookiePolicies, DiskCache,    DNSPrefetch,
    FileURLsCrossAccess, JavaScript,     LoadImages,   PreferredLanguages,
    ShowIndicators,      StrictTLS,      ParameterLast};

static ParamName loadcommitted[] = {
    AcceleratedCanvas,
    //	AccessMicrophone,
    //	AccessWebcam,
    CaretBrowsing, DefaultCharset, FontSize, FrameFlattening, Geolocation,
    HideBackground, Inspector, Java,
    //	KioskMode,
    MediaManualPlay, Plugins, RunInFullscreen, ScrollBars, SiteQuirks,
    SmoothScrolling, SpellChecking, SpellLanguages, Style, ZoomLevel,
    ParameterLast};
...
void updatetitle(Client *c) {
  ...
  if (curconfig[ShowIndicators].val.i) {
    ...
    if (c->progress != 100)
      title = g_strdup_printf("[%i%%] %s:%s | %s", c->progress, togglestats,
                              pagestats, name);
    else
...
WebKitWebView *newview(Client *c, WebKitWebView *rv) {
  ...
  if (rv) {
    ...
    settings = webkit_settings_new_with_settings(
        ...
        curconfig[DNSPrefetch].val.i, "enable-frame-flattening",
        curconfig[FrameFlattening].val.i, "enable-html5-database",
        curconfig[DiskCache].val.i, "enable-html5-local-storage",
        curconfig[DiskCache].val.i, "enable-java", curconfig[Java].val.i,
        "enable-javascript", curconfig[JavaScript].val.i, "enable-plugins",
        curconfig[Plugins].val.i, "enable-accelerated-2d-canvas",
        curconfig[AcceleratedCanvas].val.i, "enable-site-specific-quirks",
    ...
    if (strcmp(fulluseragent, "")) {
      ...
      webkit_settings_set_user_agent_with_application_details(settings, "Surf",
                                                              VERSION);
    }
    ...
    webkit_web_context_set_preferred_languages(
        context, curconfig[PreferredLanguages].val.v);
    webkit_web_context_set_spell_checking_languages(
        context, curconfig[SpellLanguages].val.v);
    webkit_web_context_set_spell_checking_enabled(
        context, curconfig[SpellChecking].val.i);
    ...
    v = g_object_new(WEBKIT_TYPE_WEB_VIEW, "settings", settings,
                     "user-content-manager", contentmanager, "web-context",
                     context, NULL);
  }
  ...
  case GDK_KEY_PRESS:
    if (!curconfig[KioskMode].val.i) {
      for (i = 0; i < LENGTH(keys); ++i) {
        if (gdk_keyval_to_lower(e->key.keyval) == keys[i].keyval &&
            CLEANMASK(e->key.state) == keys[i].mod && keys[i].func) {
          updatewinid(c);
...
void clipboard(Client *c, const Arg *a) {
  if (a->i) { /* load clipboard uri */
    ...
    gtk_clipboard_set_text(gtk_clipboard_get(GDK_SELECTION_PRIMARY),
                           c->targeturi ? c->targeturi : geturi(c), -1);
  }
}
...
static void msgext(Client *c, char type, const Arg *a) {
  ...
  if ((ret = snprintf(msg, sizeof(msg), "%c%c%c%c", 4, c->pageid, type,
                      a->i)) >= sizeof(msg)) {
...
void scrollv(Client *c, const Arg *a) { msgext(c, 'v', a); }

void scrollh(Client *c, const Arg *a) { msgext(c, 'h', a); }
...
void stop(Client *c, const Arg *a) { webkit_web_view_stop_loading(c->view); }
